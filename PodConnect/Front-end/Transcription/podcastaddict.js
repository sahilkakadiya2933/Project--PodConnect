...
document.addEventListener('DOMContentLoaded', async (event) => {
    ...
    const response = await fetch('http://localhost:8000/directlink/', {
        method: 'POST',
        body: formData,
    });
...
prevBtn.addEventListener("click", () => {
    // Logic for previous button goes here
});
...
document.getElementById("downloadSrt").addEventListener("click", async (event) => {
    event.preventDefault();
    await sendPostRequest('srt');
});

document.getElementById("downloadVtt").addEventListener("click", async (event) => {
    event.preventDefault();
    await sendPostRequest('vtt');
});

document.getElementById("downloadTxt").addEventListener("click", async (event) => {
    event.preventDefault();
